...
public class FilingRenderer : MonoBehaviour
{
    ...
    [SerializeField]
    public List<RawTracker> trackers;
    ...
    void Start()
    {
        ...
        for (int i = 0; i < sim.magnets.Length; i++)
        {
            ...
            if (i < trackers.Count && trackers[i] != null)
            {
                magnets[i].GetComponent<PositionTracker>().target = trackers[i];
                magnets[i].GetComponent<RotationTracker>().target = trackers[i];
            }
        }
