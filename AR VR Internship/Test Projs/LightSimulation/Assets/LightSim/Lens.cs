...
public class Lens : MonoBehaviour
{
    ...
    public void Update()
    {
        if (type == Type.ConvexLens) CalculateFocalPoints();
    }

    private void Bake()
    {
        SphereCollider s1 = front.GetComponent<SphereCollider>();
        SphereCollider s2 = front.GetComponent<SphereCollider>();
        r1 = s1.radius * Mathf.Max(front.transform.localScale.x, front.transform.localScale.y, front.transform.localScale.z); //radius of first sphere
        r2 = s2.radius * Mathf.Max(back.transform.localScale.x, back.transform.localScale.y, back.transform.localScale.z); //radius of second sphere
    ...
    public bool verifyHit(LightRayHit hit, ref bool backface)
    {
        switch (type)
        {
            case Type.ConcaveLens:
                {
                    backface = !backface;
                    return (back.transform.position - hit.point).sqrMagnitude < bound;
                }
