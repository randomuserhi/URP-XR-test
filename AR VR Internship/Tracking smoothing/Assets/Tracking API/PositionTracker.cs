...
public class PositionTracker : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        ca = GetComponent<CollisionAdapter>();
        if (ca && rb) Debug.LogWarning("Don't use a RigidBody along side a CollisionAdapter!");
        useTransform = ca == null && rb == null;
    ...
    private void Track()
    {
        if (!useTransform) y = transform.position;
        ...
        if (useTransform) y += T * yd;
        yd += T * (x + k3 * xd.Value - y - k1 * yd) / k2Stable;

        if (useTransform) transform.position = y;
        else if (rb != null) rb.velocity = yd;
        else ca.velocity = yd;
    }
}
